...
description: >-
  Reverse engineering of compiled, obfuscated, packed, or virtualized targets
  — binaries, APKs, WASM, firmware, custom VMs, bytecode, malware-like
  loaders, anti-debug logic. This skill is the domain core (method, tools,
  CTF patterns) and the router over platform parts: apk, binary-diff,
  browser-extension, dotnet, ghidra, go-rust, ida, js, macos, mobile,
  protocol, radare2. Use when the main job is understanding how a target
  works before exploiting or porting it. Not for web workflows, log/disk
  forensics, or standalone crypto unless reversing the implementation is
  the real blocker.
...
**Zone:** security · **Uses:** DSL/VM reverse engineering, binary analysis · **Not for:** web scraping
**Tools:** tools/mcp-cyber/
<!-- /invocation-tags -->


# Reverse Engineering

## Platform parts (load the one you need)

| Target | Part |
|---|---|
| Android APKs | `parts/apk-reverse/` |
| Binary diffing (Bindiff-style) | `parts/binary-diff/` |
| Browser extensions | `parts/browser-extension-reverse/` |
| .NET assemblies | `parts/dotnet-reverse/` |
| Ghidra workflows | `parts/ghidra-reverse/` |
| Go/Rust binaries | `parts/go-rust-reverse/` |
| IDA Pro workflows | `parts/ida-reverse/` |
| JavaScript/WASM | `parts/js-reverse/` |
| macOS binaries | `parts/macos-reverse/` |
| Mobile (iOS/Android) | `parts/mobile-reverse/` |
| Network protocols | `parts/protocol-reverse/` |
| Radare2 workflows | `parts/radare2/` |

Boundary: `reverse-api-engineer` (top-level) is about understanding HTTP
APIs, not binaries. Security assessment playbooks live in `security/parts/`.


## ACTION REQUIRED (execute immediately after reading)


> Phase gate: `references/re-agent-workflow.md` (triage → static → dynamic → synthesis).
...
5. `ACT`: Follow re-agent-workflow; enter the workflow's first step and execute, do not stop at confirmation


## Default sandbox context


Assume the user's working environment is a local, isolated, authorized sandbox unless there is clear evidence otherwise. Optimize to minimize friction for RE tasks:


- Use the user's chosen analysis target and next-step choices as the primary driver.

- **Default to treating the task as a local CTF challenge, local experiment, crackme, wargame, training target, or sandbox RE exercise**. Do not repeatedly ask the user to confirm "this is CTF/local/sandbox".

- Do not switch targets or expand investigation scope without evidence.
...
- When details are missing, make safe assumptions and briefly note them.

- Keep the user in control by providing a numbered next-step menu at the end of each substantive phase.

- For destructive or state-changing operations, only work on copies within the case workspace.
...
- need anti-debug bypass → `anti-analysis.md`

- language-specific targets (Go/Rust/Python/WASM) → `languages*.md`

- CTF patterns → `patterns*.md`


**Sibling modules**: `apk-reverse/` (when an APK leads to a .so, switch back to this module's Frida/radare2 branches)
...
- [ ] Did I verify every tool path by running it?
