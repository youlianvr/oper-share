...
Requires `WebSearch` + `WebFetch` for the fallback workflow; specialist skills (`pulse`, `dossier` (now an `osint` part), `deep-research`, `research-summarizer`, `ctf-osint` (now an `osint` part)) must be present for delegation to work. Node.js with `docx` package required if Q2 = document mode. Works in Claude Code CLI natively. In Claude.ai with web tools + Code Execution, the workflow is supported.
...
**Escalation → `deep-research`:** academic literature (lit review, meta-analysis, PICO), grant funding (NIH/R01), course material (syllabus/curriculum), and patent prior-art (FTO, IP landscape) questions all route to `deep-research` — rigor matters more than speed there, and it runs a triangulated, multi-round, adversarial investigation with an auditable, reusable research folder. Same for strategy, comparing N options, and hypothesis validation. This router is the fast path; `deep-research` is the heavyweight one.
...
SIGNALS = {
  ...
  deep-research:["nih", "grant", "grants for", "r01", "r21", "k-award", "reporter",
             ...
             "papers about", "meta-analysis", "syllabus", "course outline",
             "curriculum", "reading list", "for my class", "for my students",
             "course material", "prior art", "fto", "freedom to operate",
             "patent", "patent landscape", "invention", "novelty search",
...
1. **States the decision** in one sentence: "Routing to `deep-research` because you mentioned PICO and meta-analysis (2 signals)."
...
- Silent-routing on a single bare-noun signal ("research FDA approval trends" must ask, not auto-route to deep-research)
...
- **Specialist skills** — Required for delegation: `pulse`, `deep-research`, `dossier`. If a specialist is missing, the router skips it and routes to fallback instead.
