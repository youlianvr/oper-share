...
| 4 | **Search** (loop) | Dispatch sources → run searches in parallel batches → fetch & dedup → save each to `sources/NN.md`; re-evaluate between rounds |
...
- **Parallel search.** Phase 4 runs searches concurrently in one batch (cheap models for broad web sweeps, stronger ones for reasoning-heavy subtopics) — never one-at-a-time.
...
- **Don't run searches sequentially.** Fan-out in parallel; serial search wastes the wall-clock advantage.
