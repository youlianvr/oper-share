...
Use when the source is a public URL, heavily dynamic (JS/SPA), requires search-first discovery, or involves bulk crawling across a domain. Firecrawl is an external SDK/API dependency and is **not installed in the audited environment**; verify the package and API key before selecting this mode.
...
Use when Firecrawl handles URL discovery/web extraction, but local Python (Pandas) is required to clean, normalize, and structure the output before saving. Do not treat the template's successful `--sample` path as proof that Firecrawl itself is configured or live.
...
- **browser-automation**: Use for highly interactive scraping requiring user emulation (clicks, logins) where a currently verified browser operation is available; Firecrawl/Playwright/Camoufox names in examples do not by themselves prove live capability.
