...
## Design spec format (merged from looper, 2026-08-09)

When the user wants the loop captured as a machine-checkable spec (not just a
prompt), use the looper YAML format. It adds budget caps and verification gates
on top of the feedback cycle above:

```yaml
goal:
  statement: "Produce X that does Y"
  definition_of_done: "Criteria that prove completion"
  verification:
    - id: build-ok
    ...
    - id: quality
      type: judge
      rubric: "Code is clean, tested, documented"
...
Design-time checklist (from looper):
1. Define the EXACT outcome and a concrete definition of done.
2. Break into steps; each step declares input, output, tool, verification.
3. Add verification gates per critical step: programmatic (exit 0), judge (rubric),
   or human (explicit confirmation).
4. Set budget caps: max_tokens / max_steps / max_time / no_progress_stop.
5. Optional cross-model review: reviewer model different from executor, scope
   (plan / delivery / both), verdict approve/reject with reasoning.

Anti-patterns: vague goals ("make it better" — unverifiable), no verification
("it should work"), no budget cap (token burn), single model (blind spots),
over-engineering (10 steps when 3 will do).
