...
2. **Design-system not onboarded.** If `~/.config/markdown-html/design-system.json` doesn't exist (or its `setup_completed_at` is null), refuse. Point the user at `python3 ../design-system/scripts/onboard.py` (or `--defaults` for a zero-touch run).
...
```bash
python3 scripts/doctype_classifier.py \
    --input <path>.md --output json \
  | python3 scripts/route_explainer.py
```
...
```bash
python3 ../design-system/scripts/onboard.py
```
...
```bash
python3 scripts/output_path_resolver.py \
...
Walk these batched by branch (grill-me mechanics), with a recommended answer per question, citing the canon. Lift this list into `grill-me` for plan-stage interrogation.
...
- Brand palette math: `../landing/scripts/brand_palette_validator.py` (WCAG + HSL derive)
