...
> `<SKILL_ROOT>` is this package's actual `skills/` directory path — detect it automatically.
...
under surfaceiscanEdit's [language] Version：
...
For English user requests, respond in English.
...
| Tool | Auto-install | Install method | Note |

|------|-----------|---------|------|

| Mermaid CLI (mmdc) | yes | npm install -g @mermaid-js/mermaid-cli | renders Mermaid to PNG/SVG |

| Graphviz (dot) | no | manual install | https://graphviz.org/download/ |

| PlantUML | ✗ | need Java + plantuml.jar | https://plantuml.com/download |

| Python (render script) | ✓ | alreadyin bootstrap in | `scripts/render_diagram.py` Dependency |


### Note


This skill mainly outputs text-format diagram source (Mermaid/DOT/PlantUML); local rendering tools are not necessarily required. Renderers are only needed when the user explicitly requests PNG/SVG/PDF files.


If a renderer is unavailable, `scripts/render_diagram.py` outputs install hints instead of erroring.
...
**Trigger condition**: the user asks for a diagram — "draw a diagram", "flow diagram", "architecture diagram", "attack path diagram", "sequence diagram", "Mermaid", "Graphviz", "PlantUML"

**Downstream exit**:

- The generated diagram can be embedded in a `docs-generator/` report

- Attack-path diagrams pair with `pentest-tools/` reports


**Sibling modules**: `docs-generator/` (embeds diagrams in reports)
