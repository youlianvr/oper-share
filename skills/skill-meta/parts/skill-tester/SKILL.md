...
Meta-skill that validates, tests, and scores skills in this repository. Four tools, run from the **verified skill root** (not a guessed historical `engineering/` path) with full paths:
...
```bash
# 1. Validate structure (read-only; exit non-zero on failure)
python3 .agents/skills/skill-meta/parts/skill-tester/scripts/skill_validator.py .agents/skills/<skill-name> --json

# 2. Test the skill's Python scripts (30s default timeout per script)
python3 .agents/skills/skill-meta/parts/skill-tester/scripts/script_tester.py .agents/skills/<skill-name> --json

# 3. Score quality (fail CI below threshold with --minimum-score)
python3 .agents/skills/skill-meta/parts/skill-tester/scripts/quality_scorer.py .agents/skills/<skill-name> --json --detailed --minimum-score 75
```
...
For repo-wide auditing prefer the verified local path `.agents/skills/skill-meta/parts/skill-tester/scripts/audit_skills.py` (bundled with this skill; wraps the write-a-skill checklist runner across all skills). The bundled legacy validator/scorer may reject current repository-native frontmatter or imports; treat such output as `FAILED-BOUNDED` contract evidence, not as loader failure, until the validator is updated.
...
- name: "validate-changed-skills"
  run: |
    for skill in $changed_skills; do
      python3 scripts/skill_validator.py "$skill" --json
      python3 scripts/script_tester.py "$skill"
      python3 scripts/quality_scorer.py "$skill" --minimum-score 75
    done
```
