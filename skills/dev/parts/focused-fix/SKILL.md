...
- **REQUIRED SUB-SKILL:** For complex bugs found during diagnosis, apply `systematic-debugging` Phase 1 (Root Cause Investigation) to confirm before proceeding
...
- **`systematic-debugging`** — Use within Phase 3 for root-cause tracing of individual complex bugs
- **`verification-before-completion`** — Use within Phase 5 before claiming the feature is fixed
...
## Authorization and scope (merged from implement)

- **Loading this skill is not permission to write by itself.** The right
  to write is given by an authorized request or an approved plan.
- **Do not expand scope silently.** Any exit beyond the feature folder —
  with explicit notification to the user.
