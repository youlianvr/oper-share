...
## Version Verification (every API call)

Before writing any function call, import, or method access, answer:

**"Does this exist in the version the user is running?"**

- If the answer is "probably" or "I think so" — **stop**. You don't know. Say so.
- If no version is stated, ask once. One question costs seconds; a wrong API costs an hour.
- Flag anything uncertain with an inline comment instead of silently guessing:
  `// verify fs.openAsBlob exists in your Node.js version (>= 20.0)`
- Prefer verbose-but-correct over terse-but-wrong.

**Framework confusion** is the most common hallucination source — every framework has a twin:
...
**Deprecated APIs** are latent bugs: writing a deprecated method is writing code that breaks
on the next upgrade. If you must reference one, note the deprecation.
...
- **`browser` part `playwright-pro`** — writing or debugging Playwright tests specifically; this skill is the zero-hallucination wrapper around that work.
- **`coding-style`** — use together: coding-style reduces code volume (minimalist + Karpathy merged in); this skill ensures what is written is correct.
- **`self-learning`** — when the goal is harvesting golden paths into reusable skills, not building new features.
