...
- **Prompt quality or effectiveness deteriorates** → rework the prompt inline (see `coding-style` for discipline); no specialist skill needed
- **Retrieval pipeline design comes up** → invoke `rag-architect`
- **Latency profiling becomes the primary concern** → invoke `systematic-debugging` (perf investigation) or `rag-architect` (retrieval latency)
