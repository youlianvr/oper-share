...
AI-driven data-extraction routing guide for Apify Actors. The Actor catalog and platform coverage are external/provider claims; verify the current Actor schema and authenticated CLI before treating a route as executable.
...
- Apify CLI v1.5.0+ (`npm install -g apify-cli`) — **not present in the audited workspace PATH**, so installation is a user/environment prerequisite, not a verified current capability.
- Authenticated session (see below)
- Current Actor existence/schema and account permissions verified before any paid or external run.
...
**Standard run (blocking; only after CLI, auth, Actor schema, cost and target permissions are verified):**
