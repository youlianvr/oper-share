...
name: antislop
description: >-
  Anti-slop filter for AI work. One skill, six parts: UI, copywriting, people
  and accessibility, mobile layout, code comments, prose editing. Read the core
  rules here; open only the part the task needs.
...
## Parts — open the one the task needs

This file is the core filter and is complete on its own for interface work. The
parts below live in `references/` next to this file; open only what the task touches.

| Task | Reference |
|---|---|
| Building or editing an interface: color, layout, components, decoration, motion | `references/ui.md` |
| Writing or editing copy: headlines, CTAs, value propositions, tone, product prose | `references/copywriting.md` |
| Making a UI work for different eyes, hands and setups: contrast, keyboard, focus, states | `references/people-and-accessibility.md` |
| Layouts that must hold up on a phone: breakpoints, scale, grids, overflow, tap targets | `references/layout-mobile.md` |
| Code comments: remove generic AI-slop comments, keep the valuable ones, never touch the code | `references/code.md` |
| Editing prose that already exists, in the writer's own voice, or detecting AI-slop patterns | `references/prose-editing.md` |

The contrast checker that shipped with the former `antislop-human` skill is
`scripts/contrast-check.py` (and `scripts/contrast-mcp.py`, an MCP wrapper).
