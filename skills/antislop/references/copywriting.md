...
### Self-Description of the Build

- **The pattern:** a product page spending its space on how the product was made instead of what it serves: "built with clean code and no frameworks", "this website works offline", "the data lives in one JSON file", "developed using modern technologies and best practices". Includes self-assessment of one's own qualities: "the site is fast, honest, reliable, convenient".
- **Why it reads as AI (and as a school project):** the reader came for the content, not for the workshop tour. The tech stack is the author's concern; praising it on the user-facing page is self-congratulation that fills a section without serving anyone. Judges and users both recognize it instantly.
- **Before:**
  > The site is built with clean HTML, CSS and JavaScript, without build tools or libraries. It works offline, so the page does not require an internet connection.
- **After:**
  > (Cut the paragraph. If a technical fact has a user-facing consequence, state the consequence, not the craft: "works offline" stays only if the reader needs to know that; "no build tools" never stays.)
- **Rule:** R-36, C-3 (content-driven composition), C-5.
- **Not a ban:** an "About the project" page for a contest may carry task framing and sources; licensing and attribution remain. The ban is on describing and praising the making.
...
- [ ] No self-description of the build: the page serves content, not a tour of its own tech stack; no self-assessed qualities (fast, honest, clean) without evidence a reader needs (R-36, C-3)
