...
python3 .agents/skills/ship/parts/env-secrets-manager/scripts/env_auditor.py /path/to/repo

# JSON output for CI pipelines
python3 .agents/skills/ship/parts/env-secrets-manager/scripts/env_auditor.py /path/to/repo --json
```
...
1. Run `.agents/skills/ship/parts/env-secrets-manager/scripts/env_auditor.py` on the repository root.
...
> **Cross-reference:** `engineering/secrets-vault-manager` is an external/example reference; verify that it exists in the selected project before routing to it.
...
- Use `.agents/skills/ship/parts/env-secrets-manager/scripts/env_auditor.py` to flag secrets with no recorded rotation date.
...
|-------|------|-------------|
| **Secrets Vault Manager** | `engineering/secrets-vault-manager` (external/example) | Production vault infrastructure, HA deployment, DR; not an active local skill path |
