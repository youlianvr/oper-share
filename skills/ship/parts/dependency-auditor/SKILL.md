...
```bash
# 1. Scan for vulnerabilities (built-in offline CVE pattern set; read-only)
# Run from this skill directory or replace `scripts/` with the verified skill path.
...
**Verification loop:** after an explicitly authorized upgrade, re-run step 1 and assert 0 high-severity findings before closing the audit. This skill's scan is read-only; applying upgrades is outside the audit default.
...
```bash
# Security gate in CI (execute from the verified skill directory or use an absolute verified script path)
