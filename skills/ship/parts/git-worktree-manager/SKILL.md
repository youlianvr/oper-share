...
- Optionally copy local environment files (`.env*`) only with explicit authorization; never print, commit, or expose their contents
- Optionally install dependencies based on lockfile detection, only after explicit authorization; read-only inspection is the default
...
python scripts/worktree_manager.py \
  ...
  --name wt-auth \
  --base-branch main \
  --format text  # inspect/create only; dependency installation requires separate explicit authorization
```
...
3. Report merged + clean worktrees; removal is a separate explicitly authorized mutation and must use the repository's safe trash/recycle policy, never force removal by default.

```bash
python scripts/worktree_cleanup.py --repo . --stale-days 14 --format text
python scripts/worktree_cleanup.py --repo . --format text  # inspect only; --remove-merged requires explicit authorization and a separate controlled action
```
...
- `python scripts/worktree_manager.py --help`
  ...
  - Report whether `.env*` sync would be needed (copy only with explicit authorization)
  - Optional dependency installation (explicit authorization required)
...
- If dependency install fails: keep worktree created, mark status and continue manual recovery; do not retry installation without authorization.
