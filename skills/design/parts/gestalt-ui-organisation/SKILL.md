...
Default to **whitespace, not divider lines**, for grouping. Remove unnecessary separators.
...
**Borderless regions need spacing.** Enclosure can come from a border/fill or whitespace alone. When a card has no border or background, use generous internal padding and a clear gap to its neighbours to distinguish it as one region.
