...
For several brands, use the same semantic tokens; change only primitives (colour, type, radius) per brand.


**Consolidate periodically.** Schedule a recurring consolidation pass:
...
- **Set a design direction.** Define the intended UX and visual direction for each product during consolidation.
...
**Deprecate on touch.** Keep a short list of deprecated styles — the old radius, the old shadow, the retired button variant. When work already touches an old-generation page, lift it to the current style in the same pass.


**Two or three occurrences make a pattern.** When the same visual treatment appears independently in 2–3 places, name it, tokenise it, and make it available everywhere. New components may use it as-is or adapt it, as long as the adaptation stays recognisably true to the original.


**Track feature usage.** Instrument what gets used, and deprecate features/components with little real usage.
