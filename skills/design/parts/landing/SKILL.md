---
name: landing
description: "Generates a premium single-page HTML landing page with 3D CSS animations, GSAP scroll effects, and mouse-parallax depth. Forcing intake (product + elevator pitch, audience register, brand overrides, tone) locks down positioning before any copy or markup is written, so the page reflects the actual product rather than generic boilerplate. Use whenever the user says 'landing for X', 'create a premium HTML landing page', 'build a single-file landing', 'make a landing page for X', 'one-pager', 'I need a web page for Y', or provides product/service details and wants a polished standalone website. NOT for React/Next.js + Tailwind landings (use landing-page-generator). NOT for cinematic 2.5D scroll effects (use epic-design). Also triggers on 'promotional page', 'product page', 'one-pager', 'web presence', 'sales page'. Outputs a single self-contained HTML file (Claude Code) or HTML artifact (Claude.ai). Supports configurable brand colors via CSS custom property overrides."
license: MIT
metadata:
...
Generate one `.html` landing page from a text prompt or brief. Keep all CSS in `<style>` and all custom JS in `<script>`; the only external dependencies are Google Fonts and GSAP via CDN.
...
>
> *Why I'm asking:* The headline, subtext, and feature copy derive from this. "App for productivity" is vague; "Async standup tool for remote engineering teams who hate Zoom" specifies the function and audience.

**Require a pitch.** If user gives just a name with no pitch, push back once: "What does it do? Who's it for?" If still no pitch after push-back, deliver with explicit "generic positioning" caveat.
...
>
> *Why I'm asking:* Overrides match the page to your existing brand; a primary color alone is enough to derive a palette.
