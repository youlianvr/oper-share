...
Derive a brand palette from default or partial user input and check WCAG contrast minimums.

Pair with `scripts/brand_palette_validator.py` for the deterministic implementation.

## The Default Palette (Dark Navy + Teal)

The default uses dark backgrounds and a teal accent. `#F7F7F2` text on `#0A1628` bg is ~17:1 contrast (WCAG AAA for both small and large text).
