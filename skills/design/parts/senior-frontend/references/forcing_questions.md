...
**Why it's the first question:** use the target device and network constraints to choose rendering, bundle, and image budgets.

**Kill criterion:** "all users equally" — STOP. Pull analytics or target-market data.
...
**Why it matters:** use the LCP target to scope performance work and measure its effect. See Akamai 2017 and Chrome UX Report 2024 in the sources below; measure conversion effects for this product rather than assuming a fixed lift.
...
**Why it matters:** rendering determines data fetching, state management, hydration, and server costs. Make the choice explicit before adopting a framework default.
...
**Why it matters:** enforce a bundle ceiling in CI to detect growth that threatens Q2's LCP target. Tim Kadlec's *Performance Budgets* (2013) framing: set the ceiling and fail the build when it is crossed.
...
**Why it matters:** an SEO-dependent surface must render content in HTML and meet Core Web Vitals targets. An auth-walled surface can defer SSR, but its bundle budget still depends on Q1 and Q2.
...
**Why it matters:** a shared source of truth keeps components and tokens consistent across contributors. For teams of three or fewer, start with shadcn/ui + Tailwind tokens before investing in a custom system.
...
**Why it matters:** identify applicable accessibility obligations before implementation, including the European Accessibility Act and US ADA Title III where relevant. Assign a named owner to track accessibility work and verification.
