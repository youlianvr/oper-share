...
description: >
  ...
  Use for CINEMATIC web design: scroll storytelling, parallax depth, text animations,
  hero sections, sticky sections, section overlaps, floating products, clip-path
  reveals, text that flies in from sides, words that light up on scroll, curtain
  drops, iris opens, card stacks, bleed typography, and any site that should feel
  cinematic or premium. Trigger on phrases like "make it feel
  ...
  Use aggressively for cinematic scroll-driven work. Covers 45+ techniques across 8 categories. Always inspects, judges, and plans assets before coding. NOT for a plain landing page (use landing for single-file HTML, landing-page-generator for React/Tailwind).
license: MIT
metadata:
  version: 1.0.0
  author: Abbas Mir
...
Build cinematic 2.5D websites using flat PNG/static assets, CSS, and JavaScript. No WebGL or 3D modeling software required.
...
→ Full boilerplate: the inline example above (Step 5) is the complete hero section
→ CSS system: see `references/motion-system.md` + `references/depth-system.md`
→ JS engine: see `scripts/validate-layers.js` (depth/scroll validation)
...
- **landing**: Use for a plain single-file HTML landing page. NOT for custom cinematic experiences.
- **frontend-design**: Use for general visual design direction. NOT for scroll animations or depth effects.
- **a11y-audit**: Use to verify full WCAG compliance after build.
