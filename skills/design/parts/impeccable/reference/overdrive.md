...
2. **STOP and use Codex's structured user-input/question tool when available; otherwise ask directly in chat.** Present these directions, clarify what you cannot infer, and get the user's pick before writing any code. Explain trade-offs (browser support, performance cost, complexity).
