...
- The first implementation of a new world is complete and its provisional decisions need to be documented.
- An existing `DESIGN.md` is stale (the design has drifted).
- Before a large redesign, to capture the current state as a reference.

If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file, then STOP and ask whether to refresh, overwrite, or merge. Use Codex's structured user-input/question tool when available; otherwise ask directly in chat. Wait for the user's answer.
